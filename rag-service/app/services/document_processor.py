...
class DocumentProcessor:
    ...
    async def process_document(self, 
                               ...
                               filename: str, 
                               file_content: bytes,
                               metadata: dict = None) -> BrandDocument:
        ...
        chunks_data = self.text_splitter.split_with_metadata(text_content, str(document.id))
    ...
    async def get_brand_documents(self, 
                                  db: AsyncSession, 
                                  brand_id: str) -> List[BrandDocument]:
    ...
    async def get_document(self, 
                          db: AsyncSession, 
                          document_id: str) -> BrandDocument:
        """Get a specific document"""
        result = await db.execute(
            select(BrandDocument)
            .where(BrandDocument.id == document_id)
        )
    ...
    async def get_brand_chunk_count(self, 
                                   db: AsyncSession, 
                                   brand_id: str) -> int:
