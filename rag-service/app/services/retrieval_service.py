...
class RetrievalService:
    ...
    async def search_similar_chunks(
        self, db: AsyncSession, brand_id: str, query: str, limit: int = None
        ...
        result = await db.execute(
            sql, {"brand_id": brand_id, "embedding": embedding_str, "limit": limit}
        )
    ...
    async def _keyword_search(
        self, db: AsyncSession, brand_id: str, query: str, limit: int
    ) -> List[Tuple[DocumentChunk, float]]:
        ...
        result = await db.execute(
            select(
                ...
                func.sum(
                    func.case(
                        *(
                            (DocumentChunk.content.ilike(f"%{keyword}%"), 1)
                            for keyword in keywords
                        ),
                        else_=0,
                    )
